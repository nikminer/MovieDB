...
for i in models.Serial.objects.all():
    ...
    for ul in models.UserList.objects.filter(serial=i):
        WLmodels.WatchList.objects.create(user=ul.user,movie=movie, userrate=ul.userrate, userstatus=ul.userstatus, rewatch=ul.countreview, userepisode=ul.userepisode, updated=ul.updated)
...
for i in models.Film.objects.all():
    ...
    for ul in models.UserListF.objects.all():
